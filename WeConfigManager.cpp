...
static std::wstring Utf8ToWstring(const std::string& str)
{
    ...
    MultiByteToWideChar(CP_UTF8, 0, str.data(), static_cast<int>(str.size()), wide.data(), sizeNeeded);
    return wide;
}
...
bool PkgParser::Parse(const std::wstring& filePath)
{
    ...
    if (versionStr.size() < 4 || std::strncmp(versionStr.c_str(), "PKGV", 4) != 0)
        return FailAndCleanup(L"魔数校验失败，实际读取到: " + Utf8ToWstring(versionStr));
...
PkgParser::RgbaImage PkgParser::DecodeTexvToRGBA(const MemSpan& texSpan) const
{
    ...
    const uint8_t* const searchEnd = searchStart + (std::min<size_t>)(texSpan.size - sizeof(TexvHeader), 4096);
...
bool PkgParser::CalcStatsFromRgba(const RgbaImage& img, double wPct, double hPct, double& outRoiAvg,
                                  double& outRoiDark, double& outGlobalAvg, double& outGlobalDark) const
{
